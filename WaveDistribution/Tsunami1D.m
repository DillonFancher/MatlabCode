      ...
      for i = 2 : n
          ...
          if i == n
              if beta == 0
                  ...
                  phantom = rightBoundary(n, alpha, beta, gamma, deltaX, uCur);
                  uFut(n) = ( lamb*((-deltaX*b/2)+a)*uCur(n-1) + (2-2*lamb*a)*uCur(n) + lamb*((deltaX*b/2)+a)*phantom - uInit(n));
              end
          else
              uFut(i) = ( lamb*((-deltaX*b/2)+a)*uCur(i-1) + (2-2*lamb*a)*uCur(i) + lamb*((deltaX*b/2)+a)*uCur(i+1) - uInit(i));
          end
      end
      % input('delay')
